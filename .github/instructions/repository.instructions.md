---
applyTo: "src/MenuHelper.Infrastructure/Repositories/*.cs"
---
...
- 仓储接口和实现应放置在 `src/MenuHelper.Infrastructure/Repositories/` 目录下。
- 文件名格式为 `{AggregateName}Repository.cs`。

## 代码示例

**文件**: `src/MenuHelper.Infrastructure/Repositories/AdminUserRepository.cs`

```csharp
using MenuHelper.Domain.AggregatesModel.AdminUserAggregate;
