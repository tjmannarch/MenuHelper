<template>
	<view class="page">
		...
		<wd-cell-group custom-class="form-group">
			<wd-cell title="菜品名称" required>
				<template #value>
					<wd-input v-model="form.name" placeholder="请输入菜品名称" no-border />
				</template>
			</wd-cell>
		</wd-cell-group>
		...
		<template v-if="isEdit">
			...
			<wd-cell-group v-if="dishIngredients.length > 0" custom-class="form-group">
				<wd-cell
					v-for="di in dishIngredients"
					:key="di.id"
					:title="getIngredientName(di.ingredientId)"
				>
					<template #label>
						<view class="di-label">
		...
		<view class="btn-area">
			...
			<wd-button v-if="isEdit" block type="error" plain custom-class="delete-btn" @click="confirmDelete">删除菜品</wd-button>
		</view>
		...
		>
			<view class="sheet-content">
				<wd-search v-model="ingredientKeyword" placeholder="搜索原材料" @search="searchIngredients" @clear="searchIngredients" />
				<scroll-view scroll-y style="max-height: 600rpx;">
					<wd-cell-group>
						<wd-cell
							...
							:key="ing.id"
							:title="ing.name"
							:label="categoryLabel(ing.category)"
							clickable
...
async function loadAllIngredients() {
  try {
    const res = await api.get('/api/ingredients', { pageSize: 200 })
...
async function selectIngredient(ing) {
  ...
  // 询问用量类型
  try {
    const resp = await messageBox.prompt({
      title: `配置 "${ing.name}" 用量`,
      msg: '请输入固定用量（留空表示非固定）',
      inputValue: ''
    })

    const inputVal = resp?.value ?? ''
    const fixedQty = inputVal !== '' ? parseFloat(inputVal) : null
    ...
    if (e !== 'cancel') toast.error(e?.message || '已取消')
  }
}
